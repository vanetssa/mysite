 ...
 *  // 설정값 대로 섬네일을 생성한다.
 *  self::setOption('export', EXPORT_PNG);// 섬네일 출력을 PNG 로 변경 (옵션을 지정하지 않으면 jpg로 출력)
 *  self::create('image.jpg',
...
class Thumbnail
{
	...
	public static function create($filepath, $width = null, $height = null, $scale = 'exactfit', $options = null)
	{
		...
		{
			return false;
			self::raiseError('#Error: self::create() : File not found or permission error.'.' at '. __LINE__);
		};
		...
		{
			return false;
			self::raiseError('#Error: self::create() : Invalid thumbnail size.'.' at '. __LINE__);
		}
		...
		// 기타 옵션
		if ($options)
			$options = array_merge(self::$_TdefaultOptions, $options);

		// 옵션 중 출력 이미지 형식이 잘못 지정된 경우
		if ( ! in_array($options['export'], Array('jpg', 'gif', 'png'))){
			return false;
			self::raiseError('#Error: self::create() : Invalid export format.'.' at '. __LINE__);
		}
		...
		{
			return false;
			self::raiseError('#Error: self::create() : Filetype not supported. Thumbnail not created.'.' at '. __LINE__);
		}
...
//			call_user_func($options['postprocess'], &$thumbnail, $width, $height, $thumb_width, $thumb_height);
		...
		if ($iserror){
			return false;
			self::raiseError('#Error: self::create() : invalid path or permission error.'.' at '. __LINE__);
		}
		elseif (self::getOption('debug'))
		{
			echo '@Debug: self::create() - source='. $filepath .', image[width='. $image_width .',height='. $image_height .'], '
	...
	public static function write($filepath, $width = null, $height = null, $scale = 'exactfit', $options = null)
	{
		// 원본 이미지가 없는 경우
		if ( ! file_exists($filepath))
		{
	...
	{
		self::$_TdefaultOptions[ $name ] = $value;
	}
	...
	{
		return self::$_TdefaultOptions[ $name ];
	}
